...
log_file = r'E:\Machine_Learning\Project\LiSA\run\lisa_facebookpagepage.log'
...
if __name__ == "__main__":
    ...
    dataset_cls = FacebookPagePage(dataset_dir, transform=T.NormalizeFeatures())
    ...
    dataset_link = FacebookPagePage(dataset_dir, transform=transform)
    ...
    for victim_node in victim_nodes:
        ...
        sub_adj, sub_feat = model.train_subgraph(x=x,
                                                 ...
                                                 beta=beta,
                                                 lr=lr,
                                                 edge_perturbations=num_edge_perturbations,
                                                 n_links=num_recommend_users,
        ...
        train_link_adj = utils.edge_index_2_adj(x.shape[0] + sub_size, train_link_edge_index)
        ...
        test_link_adj = utils.edge_index_2_adj(x.shape[0] + sub_size, test_link_edge_index)
        test_link_adj_norm = utils.normalize_adj_tensor(test_link_adj)
        # Label and node mask on merged graph.
        label = torch.cat([data_cls.y, torch.zeros(sub_size, dtype=data_cls.y.dtype, device=device)])
        train_cls_mask = torch.cat([train_cls_mask, torch.zeros(sub_size, dtype=torch.bool, device=device)])
        val_cls_mask = torch.cat([val_cls_mask, torch.zeros(sub_size, dtype=torch.bool, device=device)])
        test_cls_mask = torch.cat([test_cls_mask, torch.zeros(sub_size, dtype=torch.bool, device=device)])
        # Merge graph.
